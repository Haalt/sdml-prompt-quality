...
class PromptQualityModelV3ClipEmbed(nn.Module):
    """
    Prompt-quality scorer that consumes pooled CLIP embeddings directly.

    Interface (all tensors are batch-first):
        clip_emb       : (B, d_clip) float32  (pooled CLIP embedding)
        lora_ids       : (B, L)      int64
    ...
    def __init__(
        ...
        # token φ (project pooled CLIP embeddings)
        self.model_token_proj = nn.Linear(d_m, d_clip)
        self.token_phi = nn.Sequential(
            nn.Linear(d_clip, d_proj),
            nn.Dropout(0.2),
            nn.ReLU(),
        )
    ...
    def forward(
        ...
        # pooled CLIP path
        v_tokens = clip_emb.to(v_dom.dtype)
        v_tokens = v_tokens + self.model_token_proj(v_dom).to(v_tokens.dtype)
        v_tokens = self.token_phi(v_tokens)
