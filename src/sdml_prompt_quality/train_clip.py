...
from .models.v3_clip import PromptQualityModelV3Clip
from .metrics import collate, epoch_metrics
from .data.preprocess_clip import preprocess_clip
...
class PromptDatasetClip(Dataset):
    ...
    def __getitem__(self, idx):
        item = {
            "input_ids": torch.as_tensor(self.samples[0][idx], dtype=torch.long),
            "attention_mask": torch.as_tensor(self.samples[1][idx], dtype=torch.long),
            ...
            "target": torch.as_tensor([self.samples[14][idx]], dtype=torch.float32),
...
def train_model(
    ...
    train_loader = DataLoader(
        PromptDatasetClip(train_samples),
    ...
    model = PromptQualityModelV3Clip(
    ...
    # Optimizer
    # We only train the non-CLIP parts
    ...
    for epoch in range(num_epochs):
        ...
        if val_mse + 1e-6 < best_val_mse:
            ...
            epochs_no_improve = 0
            torch.save(best_state, "best_model_clip.pt")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train V3 with CLIP")
    ...
    (train_samples, val_samples, vocab_loras,
     vocab_samplers, vocab_upscalers) = preprocess_clip(
        dataset_input, dataset_output, TOKENIZER_FILE, clip_model_name=args.clip_model
    )
